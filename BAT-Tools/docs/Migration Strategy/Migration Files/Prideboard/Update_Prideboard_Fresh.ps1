...
function Remove-IfExists {
    ...
    foreach ($c in $vbproj.VBComponents) {
        if ($c.Name -eq $name) { $vbproj.VBComponents.Remove($c); return }
    }
}
...
Sub BuildStatusPalette()
    ...
    Const H As Double = 20
    Const BTN_W As Double = 100   ' <- was w (conflicted)
    ...
    For s = psNotDone To psReset
        ...
        Dim btnW As Double: btnW = IIf(Len(label) > 15, BTN_W * 1.3, BTN_W) ' <- local width

        Set shp = ws.Shapes.AddShape(msoShapeRectangle, x, y, btnW, H)
        With shp
