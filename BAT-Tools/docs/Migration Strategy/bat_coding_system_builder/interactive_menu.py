...
class InteractiveMenu:
    ...
    def translate_code_interactive(self):
        ...
        print("This tool helps translate customer codes to unified BAT codes.\n")
        print("Examples:")
        print("  Richmond: Plan G603, Pack |10.82, Elevation 'B, C, D'")
        print("  Holt:     Enter complete code like '167010100 - 4085'")
        print()
        
        # Ask which customer system
        print("Customer System:")
        print("1. Richmond (uses |XX.XX pack format)")
        print("2. Holt (uses PPPPPCCCSS-AAAA format)")
        print()
        
        customer_type = input("Select system [1-2]: ").strip()
        
        if customer_type == '2':  # Holt
            print("\n" + "="*80)
            print("HOLT CODE TRANSLATION")
            print("="*80)
            print("\nHolt codes have format: PPPPPCCCSS - AAAA")
            print("  Example: 167010100 - 4085")
            print("    1670  = Plan number")
            print("    1     = Elevation (1=A, 2=B, 3=C, 4=D)")
            print("    010   = Phase")
            print("    00    = Sequence")
            print("    4085  = Activity (determines item type)")
            print()
            
            holt_code = input("Enter Holt code: ").strip()
            
            if not holt_code:
                return
            
            # Parse Holt code
            parsed = self.parse_holt_code(holt_code)
            
            if not parsed:
                print("\n✗ Could not parse Holt code")
                print("  Expected format: 167010100 - 4085")
                self.pause()
                return
            
            # Build unified code
            unified_code = f"{parsed['plan_code']}-{parsed['phase_code']}-{parsed['elevation_code']}-{parsed.get('item_type_code', '1000')}"
            
            print("\n" + "="*80)
            print("TRANSLATION RESULT")
            print("="*80)
            print(f"Holt Code:     {holt_code}")
            print(f"Unified Code:  {unified_code}")
            print()
            print("Parsed Components:")
            print(f"  Plan:      {parsed['plan_code']}")
            print(f"  Phase:     {parsed['phase_code']}")
            print(f"  Elevation: {parsed['elevation_code']}")
            print(f"  Item Type: {parsed.get('item_type_code', '1000')}")
            print(f"  Activity:  {parsed.get('activity', 'N/A')}")
            
        else:  # Richmond
            print("\n" + "="*80)
            print("RICHMOND CODE TRANSLATION")
            print("="*80)
            print()
            
            plan_code = input("Plan code (e.g., 1670, G603): ").strip()
            richmond_pack = input("Pack ID (e.g., |10.82): ").strip()
            elevation_str = input("Elevation (e.g., 'B, C, D' or blank): ").strip()
            item_type = input("Item type (Framing/Siding): ").strip() or "Framing"
            
            if not plan_code or not richmond_pack:
                return
            
            try:
                # Ensure translation table is loaded
                if self.builder.translation_df is None:
                    print("\n⚠ Loading translation table...")
                    if Path("coding_schema_translation_v2.csv").exists():
                        self.builder.load_translation_table("coding_schema_translation_v2.csv")
                    else:
                        print("\n✗ Translation table not found: coding_schema_translation_v2.csv")
    ...
    def parse_holt_code(self, option_phase_number: str) -> dict:
        """
        Parse Holt's complex Option/Phase Number format
        
        Format: PPPPPCCCSS - AAAA
        Example: 167010100 - 4085
        
        Where:
          PPPPP = Plan number (16701 = Plan 1670, Elevation 1)
          CCC = Category/Phase (010 = foundation)
          SS = Sequence (00, 01, 02...)
          AAAA = Activity code (4085, 4155, etc.)
        
        Returns dict with: plan_code, phase_code, elevation_code, activity
        """
        try:
            # Split on dash to separate main code from activity
            parts = option_phase_number.split('-')
            main_code = parts[0].strip()
            activity = parts[1].strip() if len(parts) > 1 else ""
            
            # Parse main code: PPPPPCCCSS
            if len(main_code) >= 9:
                plan_raw = main_code[0:5]      # 16701
                category = main_code[5:8]      # 010
                sequence = main_code[8:10]     # 00
                
                # Extract plan and elevation from PPPPP
                # Last digit is often elevation: 16701 = Plan 1670, Elev 1
                plan_base = plan_raw[0:4]      # 1670
                elevation_digit = plan_raw[4]  # 1 (could be 1,2,3,4 for A,B,C,D)
                ...
                elevation_code = elevation_map.get(elevation_digit, '**')
                
                # Convert category to phase code format (010 -> 010.000)
                phase_major = category
                phase_code = f"{phase_major}.{sequence}0"
    ...
    def parse_pack_id_elevation(self, pack_id: str) -> tuple:
        """
        Parse pack ID to extract elevation info
        
        Examples:
          |10ABCD FOUNDATION -> pack=|10, elevation=ABCD
          |10 FOUNDATION -> pack=|10, elevation=**
          |10.82BCD OPT DEN -> pack=|10.82, elevation=BCD
        
        Returns: (pack_clean, elevation_code)
        """
        if not pack_id or pd.isna(pack_id):
            return ("", "**")
        
        pack_id = str(pack_id).strip()
        
        # Extract pack number (starts with |)
        if '|' in pack_id:
            pack_part = pack_id.split()[0]  # Get first token
            
            # Check for elevation letters at end of pack
            import re
            # Look for pattern like |10ABCD or |10.82BCD
            match = re.match(r'(\|[\d\.]+)([A-Z]*)', pack_part)
            
            if match:
                pack_clean = match.group(1)
                elevation_letters = match.group(2)
                
                if elevation_letters:
                    return (pack_clean, elevation_letters)
                else:
                    return (pack_clean, "**")
    ...
    def import_customer_database(self):
        ...
        print("2. Holt Homes (uses 9-digit code format)")
        ...
        # Step 4: Map columns (with smart defaults for Holt)
        print("\n" + "="*80)
        print("Step 4: Map columns to unified system")
        print("-" * 80)
        
        if customer_type == '2':  # Holt
            print("\nHolt system detected - using smart defaults:")
            print("  ℹ Plan/Elevation/Phase will be parsed from 'Option/Phase Number'")
            print("  ℹ Pack ID will be extracted from 'Pack ID / Elevation(s)' column")
            print()
            
            column_map = {
                'holt_code': 'Option/Phase Number',  # Complex code
                'pack_id': 'Pack ID / Elevation(s) / Pack-Option Name',  # Pack with elevation
                'vendor_sku': 'Sku',
            ...
            print("Press Enter to accept defaults, or type column name to change:\n")
            
            override = input(f"  Holt code column [{column_map['holt_code']}]: ").strip()
            if override:
                column_map['holt_code'] = override
            
            override = input(f"  Pack ID column [{column_map['pack_id']}]: ").strip()
            if override:
                column_map['pack_id'] = override
                
            override = input(f"  Vendor SKU [{column_map['vendor_sku']}]: ").strip()
            if override:
                column_map['vendor_sku'] = override
                
            override = input(f"  Description [{column_map['description']}]: ").strip()
            if override:
                column_map['description'] = override
                
            override = input(f"  Quantity [{column_map['quantity']}]: ").strip()
            if override:
                column_map['quantity'] = override
                
            override = input(f"  Unit [{column_map['unit']}]: ").strip()
            if override:
                column_map['unit'] = override
            
        else:  # Richmond or Custom
        ...
        if missing_cols:
            print(f"\n✗ Column(s) not found in spreadsheet:")
        ...
        for idx, row in df.head(sample_size).iterrows():
            try:
                if customer_type == '2':  # Holt
                    # Parse complex Holt code
                    holt_code = str(row[column_map['holt_code']]) if pd.notna(row[column_map['holt_code']]) else ""
                    
                    # Handle multiple codes (comma-separated)
                    if ',' in holt_code:
                        # Take first code
                        holt_code = holt_code.split(',')[0].strip()
                    ...
                    if not parsed:
                        errors.append({
                            'row': idx,
                            'error': f"Could not parse Holt code: {holt_code}"
                        })
                        continue
                    
                    plan_code = parsed['plan_code']
                    phase_code = parsed['phase_code']
                    elevation_code = parsed['elevation_code']
                    
                    # Also parse pack ID for additional elevation info
                    pack_id_raw = str(row[column_map['pack_id']]) if pd.notna(row[column_map['pack_id']]) else ""
                    pack_clean, pack_elevation = self.parse_pack_id_elevation(pack_id_raw)
                    
                    # Use pack elevation if more specific
                    if pack_elevation != "**":
                        elevation_code = pack_elevation
                    
                    # Determine item type from activity code
                    activity = parsed['activity']
                    item_type_code = "1000"  # Default to framing
                    if activity == "4155":
                        item_type_code = "2100"  # Siding
                ...
                # Build unified code
                unified_code = f"{plan_code}-{phase_code}-{elevation_code}-{item_type_code}"
                
                translations.append({
                    'row': idx,
                    'customer_code': holt_code if customer_type == '2' else f"{pack_id} + {elevation}",
        ...
        if translations:
            ...
            print("-" * 80)
            print(f"{'Row':<5} {'Customer Code':<25} {'Unified Code':<25} {'SKU':<15} {'Qty':<8}")
            print("-" * 80)
            
            for t in translations[:10]:
                cust_code = t['customer_code'][:24] if len(t['customer_code']) > 24 else t['customer_code']
                print(f"{t['row']:<5} {cust_code:<25} {t['unified_code']:<25} {t['sku']:<15} {t['quantity']:>6.1f} {t['unit']}")
        ...
        if success_rate < 50:
            ...
            print("  Recommendation: Review column mappings and code parsing.")
            print()
        ...
        if choice == 'Q':
            ...
            # Add plans first
            if customer_type == '2':  # Holt
                # Extract unique plans from sample
                unique_plans = set(t['plan_code'] for t in translations)
                cursor = self.builder.conn.cursor()
                for plan in unique_plans:
                    ...
                    """, (plan, f"Plan {plan}"))
                self.builder.conn.commit()
                print(f"  Added {len(unique_plans)} plans to database")
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    self.builder.add_material(
                        ...
                        quantity=quantity,
                        unit=unit,
                        richmond_pack_id=pack_id_raw if customer_type == '2' else pack_id,
