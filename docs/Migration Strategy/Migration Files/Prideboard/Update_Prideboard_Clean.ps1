param(
    ...
    [int]$ColWBJobID = 1,   # A
    [int]$ColWBPack  = 4,   # D
    [int]$ColWBDate  = 6,   # adjust to your layout
    [int]$ColWBStatus= 7    # adjust to your layout
)

function Remove-IfExists {
    param($vbproj, [string]$name)
    foreach ($c in $vbproj.VBComponents) {
        if ($c.Name -eq $name) { $vbproj.VBComponents.Remove($c); break }
    }
}
function Get-Or-AddModule {
    param($vbproj, [string]$name)
    foreach ($c in $vbproj.VBComponents) { if ($c.Name -eq $name) { return $c } }
...
# Safe sequential replacements (no line continuations)
$modConfig = $modConfig.Replace('__WB__', $WhiteboardSheet)
$modConfig = $modConfig.Replace('__UP__', $UpcomingSheet)
$modConfig = $modConfig.Replace('__CP__', $CompletedSheet)
$modConfig = $modConfig.Replace('__CJ__', $ColWBJobID)
$modConfig = $modConfig.Replace('__CPK__', $ColWBPack)
$modConfig = $modConfig.Replace('__CD__', $ColWBDate)
$modConfig = $modConfig.Replace('__CS__', $ColWBStatus)
...
' Keep these Private to avoid duplicate-name collisions
...
# Excel must be closed; Trust Center must allow VB Project access
...
try {
    ...
    # Prune a few likely duplicate legacy modules (safe if absent)
    Remove-IfExists $vbproj 'WorkbookLocking'
    Remove-IfExists $vbproj 'WOrkbookLocking'
    Remove-IfExists $vbproj 'modWorkbookLock'

    # Install/replace modules
    Set-Module-Code (Get-Or-AddModule $vbproj 'modConfig')          $modConfig
    Set-Module-Code (Get-Or-AddModule $vbproj 'modApp')             $modApp
    Set-Module-Code (Get-Or-AddModule $vbproj 'modWorkbookLocking') $modWorkbookLocking
    Set-Module-Code (Get-Or-AddModule $vbproj 'StatusPalette')      $statusPalette
    Set-Module-Code (Get-Or-AddModule $vbproj 'modAddStatusDropdown') $modAddStatusDropdown
    Set-Module-Code (Get-Or-AddModule $vbproj 'Extractor')          $extractor
    Set-Module-Code (Get-Or-AddModule $vbproj 'UpdateWhiteboard')   $updateWhiteboard

    # Overwrite ThisWorkbook
    foreach ($c in $vbproj.VBComponents) {
