...
param(
    ...
    # Optional overrides (edit if your layout differs)
    [string]$WhiteboardSheet = "WHITE BOARD",
    [string]$UpcomingSheet   = "Upcoming Packs",
    [string]$CompletedSheet  = "Completed Packs",
    [int]$ColWBJobID = 1,   # A
    [int]$ColWBPack  = 4,   # D
    [int]$ColWBDate  = 6,   # example
    [int]$ColWBStatus= 7    # example
)

# NOTE:
# - Close Excel before running.
# - In Excel Trust Center, enable "Trust access to the VBA project object model".
# - Run this script with a per-process bypass if needed:
#   powershell -ExecutionPolicy Bypass -File .\Update_Prideboard_Revised.ps1 -WorkbookPath "C:\...\your.xlsm"
...
function Get-Or-AddModule {
    ...
    $comp = $vbproj.VBComponents.Add(1) # vbext_ct_StdModule = 1
    $comp.Name = $name
    return $comp
}
...
$modConfig = @"
Option Explicit
Public Const SHEET_WHITEBOARD As String = "$WhiteboardSheet"
Public Const SHEET_UPCOMING   As String = "$UpcomingSheet"
Public Const SHEET_COMPLETED  As String = "$CompletedSheet"

Public Const COL_WB_JOBID As Long = $ColWBJobID
Public Const COL_WB_PACK  As Long = $ColWBPack
Public Const COL_WB_DATE  As Long = $ColWBDate
Public Const COL_WB_STATUS As Long = $ColWBStatus
...
' Utilities kept Private to avoid name collisions outside this module.
...
try {
    ...
    # Remove a few common legacy modules that cause duplicates (safe if missing)
    Remove-IfExists $vbproj 'WorkbookLocking'
    Remove-IfExists $vbproj 'WOrkbookLocking'
    Remove-IfExists $vbproj 'modWorkbookLock'
    # (We keep your existing business modules; we fully-qualify to avoid conflicts anyway.)

    # Create/replace our modules
    $mConfig = Get-Or-AddModule -vbproj $vbproj -name 'modConfig'
    Set-Module-Code $mConfig $modConfig

    $mApp = Get-Or-AddModule -vbproj $vbproj -name 'modApp'
    Set-Module-Code $mApp $modApp

    $mLock = Get-Or-AddModule -vbproj $vbproj -name 'modWorkbookLocking'
    Set-Module-Code $mLock $modWorkbookLocking

    $mPalette = Get-Or-AddModule -vbproj $vbproj -name 'StatusPalette'
    Set-Module-Code $mPalette $statusPalette

    $mDrop = Get-Or-AddModule -vbproj $vbproj -name 'modAddStatusDropdown'
    Set-Module-Code $mDrop $modAddStatusDropdown

    $mExtractor = Get-Or-AddModule -vbproj $vbproj -name 'Extractor'
    Set-Module-Code $mExtractor $extractor

    $mUpdate = Get-Or-AddModule -vbproj $vbproj -name 'UpdateWhiteboard'
    Set-Module-Code $mUpdate $updateWhiteboard

    # Overwrite ThisWorkbook code-behind
    foreach ($c in $vbproj.VBComponents) {
